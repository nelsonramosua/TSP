...
// adapted for this project (writing cities instead of vertices when they're defined).
int GraphWriteDOT(const Graph* g, const char* filename, char** vertexNames) {
    ...
    for (unsigned int i = 0; i < g->numVertices; ListMoveToNext(vertices), i++) {
        ...
        if (vertexNames && vertexNames[i] && strlen(vertexNames[i]) > 0) {
            ...
            static char indexStr[16];
            snprintf(indexStr, sizeof(indexStr), "%u", i);
            label = indexStr;
        }
        ...
        for (unsigned int k = 0; k < (unsigned int)ListGetSize(edges); ListMoveToNext(edges), k++) {
            ...
            if (g->isDigraph || i <= j) {
                fprintf(f, "    n%u %s n%u", i, edgeop, j);
                if (g->isWeighted) fprintf(f, " [label=%4.2f]", e->weight);
                fprintf(f, ";\n");
            }
        }
    }
